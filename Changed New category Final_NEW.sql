...
--if EXISTS (select  Distinct * from  #temp1)
--BEGIN
--drop table #temp1;
...
case 
when tf.FNOIND  ='IND' then ceiling((vr.[VAR] + (vr.ELM*3) + vr.ADDI) + 10  )--removed +5 and replaced with +10
when tf.FNOIND  ='FNO' then ceiling((vr.[VAR] + (vr.ELM*3) + vr.ADDI) + 10  )--removed +5 and replaced with +10
when tf.FNOIND is null  AND( m1.[ Category] in ('A','B','Q')
OR m1.[ Category] like 'A%'  
OR m1.[ Category] like 'B%' 
OR m1.[ Category] like 'Q%') 
then ceiling ( (vr.[VAR] + (vr.ELM*5)+ vr.ADDI) + 10 )--removed +5 and replaced with +10
...
--convert(float,[T-4V]) + convert(float,[T-3V]) + convert(float,[T-2V]) + convert(float,[T-1V]) + convert(float,T)
...
--Var ,security ,marginreport,bhav--bhav5
